...
mat2list <- function(res,o.models=NULL,o.covariates=NULL) {
  ...
  rownames(Y$estimate)=paste("$\\beta_",paste(1:(dim(Y$estimate))[1]-1,"$",sep=""),sep="")
...
is.zelig.list <- function(x) {
  if (class(x)%in%c("strata","list")) {
    tmp <- table(unlist(lapply(x,function(z) z$call[[3]])))
    if (length(tmp)>1) {
...
# names(ml) <- c("Unemployed","Armed Forces","Population","Unemployed+Armed Forces","Armed Forces+Population","Unemployed+Armed Forces\n+Population")
# tmp <- plot.zelig.list(ml,rot=0
...
#                        ,lwd.fact=.7
# )
# tmp <- editGrob(tmp,gPath("xaxis","labels"),rot=40,just="right",gp=gpar(lineheight=.75))
# pdf(file="extra_longley.pdf",5,width=3)
# grid.draw(tmp)
# graphics.off()
